#!/usr/bin/env bash
# Start ONE benchmark vLLM engine (container: vllm-bench): drop the page
# cache, boot the engine detached, wait until it answers, print the env
# fingerprint. Assumes the box is otherwise idle — benchmarks need the
# whole GPU.
#
