...
| bench_c9518e3e96a3-r11, bench_c9518e3e96a3-r23-arm2-B65536, bench_c9518e3e96a3-r23-arm3-B65536, bench_c9518e3e96a3-r25-arm1-spec3 | 2026-08-22 | tg128 @ d16384 c1 | **mnbt 65536 — folded recipe**, mns 4, `num_speculative_tokens 3` | **103.39** | 28 pooled (R11 7 + R23 arm2 7 + R23 arm3 7 + R25 arm1 7) | R11's fold anchor, re-measured by R23 and again by R25's control. NOT pooled with the row above — different configuration. σ/med 9.87%. R25's arm read 102.81, −1.12% on the prior pooled 103.97 — inside the file's ±10% convention, so pooled. ⚠ R25 did not pre-declare this as a protection reading. ⚠ **The licensed budget comparison is R23's drift-free A-B-B-A contrast, −1.76%** (14 runs at each budget in one session), NOT the ratio of these two pooled cross-session rows. R11's single-arm +0.27% is superseded by it and agrees in verdict: **inert at c1** |
| bench_dd3afc9e1c94, bench_3d8149654d1b, bench_b20062a3c5c5-r23-arm1-A8192, bench_b20062a3c5c5-r23-arm4-A8192 / bench_c9518e3e96a3-r11, bench_c9518e3e96a3-r23-arm2-B65536, bench_c9518e3e96a3-r23-arm3-B65536, bench_c9518e3e96a3-r25-arm1-spec3 | 2026-08-22 | ctx_tg @ d16384 c1 | mnbt 8192 pre-fold / mnbt 65536 folded | **102.60** pooled / **100.73** pooled | 28 / 28 | Never scraped. The folded half gains R25's control arm (99.41, −2.41% on the prior pooled 101.87, inside ±10%); σ/med 12.47%. R23's drift-free Phase-1 budget contrast is **+4.91%**, inside R11's ±5% band by 0.09 points and ~0.9 SE — inert, and reported at the band edge rather than rounded |
| bench_ddfac4b975ed-r25-arm2-spec4, bench_93e361742c94-r25-arm3-spec5 | 2026-08-22 | tg128 and ctx_tg @ d16384 c1 | **MUTATION `num_speculative_tokens` 4 / 5**, otherwise the folded recipe (`mnbt 65536 + mns 4`) | 99.67 / 98.30 (Phase 2); 100.63 / 86.81 (`ctx_`) | 7 each | R25, deliberately diagnostic: both arms mutate a flag the recipe ships, and the round's readout is the engine's **MTP mean acceptance length** — **3.03 / 3.44 / 3.67** at `num_speculative_tokens` 3 / 4 / 5, medians of 7 logged samples, against ceilings 4 / 5 / 6. ⚠ **Raising the ceiling raises accepted length (+13.5% / +21.1%) and does NOT raise `tg` (−3.05% / −4.39% on the same-session control 102.81).** The pre-declared fold rule required ≥ +10.0% and **did not fire; `recipe.yaml` is untouched.** Prefix cache **0.0% in 7 of 7 samples on both arms**, extending R24's finding to a third and fourth lookahead size. Identical-work controls (`pp`, `ctx_pp`, `ttfr`) agree across all three arms to **≤1.4%** |
| bench_be900399e857-r25-ridealong-moetriton-crash, bench_5eea211b9a30-r25-ridealong-moefitrtllm-crash, bench_a062dab1eed0-r25-ridealong-moecutlass-crash | 2026-08-22 | — (no benchmark ran on any of the three) | **MUTATION `--moe-backend` triton / flashinfer_trtllm / cutlass** instead of the shipped `marlin` | — | 0 | R25 ride-along, run last. **THREE CRASHES at engine init, all validator refusals, none reaching a token.** `triton`: *not supported for NvFP4 MoE. Expected one of ['cutlass', 'flashinfer_trtllm', 'flashinfer_cutlass', 'flashinfer_cutedsl', 'flashinfer_b12x', 'marlin', 'humming', 'emulation']* (`fused_moe/oracle/nvfp4.py:161` — the refusal **enumerates the legal set**). `flashinfer_trtllm`: *kernel does not support current device cuda*. `cutlass`: *kernel does not support quantization scheme QuantKey(u8,scale(f8e4m3fn,static,GroupShape(row=1, col=16)),scale2(f32,static,per_tensor),symmetric)* (both `oracle/nvfp4.py:256`). ⚠ **`marlin` is what the engine's own NVFP4 oracle admits here — the alternatives are unavailable, not slower.** Excerpts in each `crash-log.txt`. Did not touch the R25 arms above |
