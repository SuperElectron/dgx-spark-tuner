# What this model is, and which flag touches which part

Read 2026-08-23 from the pinned `config.json`, `hf_quant_config.json`,
`.quant_summary.txt`, the safetensors headers (by HTTP range request), and our
own `run-0002` engine log. Figures below come from real tensor byte offsets,
not from parameter counts.

## It is not a GQA transformer

`model_type qwen3_5_moe_text`. 40 layers, `full_attention_interval 4`:

    30 linear_attention   Gated-DeltaNet, recurrent state, no KV
    10 full_attention     layers 3,7,11,...,39 — these are the only KV layers
...
**GQA is 8, not 16.** An earlier note in this tree said 16, taken from vLLM
issue #37754; that issue describes a different geometry and its flashinfer+MTP
crash report does not transfer here unchecked.

A 27-block vision tower (~0.9 GB BF16, unquantized) is resident and is dead
weight for a text benchmark.

## MoE

256 experts, top-8, `moe_intermediate_size` 512, plus a shared always-on expert
and a BF16 unquantized router. No dense layers — expert tensors exist for all
40. Active params ≈ 2.94B, and that only reaches "3B" counting `lm_head`.

Counterintuitive and load-bearing: **the MoE is not the dominant read at batch
1.** The 30 linear-attention layers read 1.016 GB per forward, more than the
entire active MoE at 0.637 GB.

## MTP

`mtp_num_hidden_layers: 1` — one layer, looped N times by vLLM, sharing
`embed_tokens` and `lm_head`. Our own log warns that running it repeatedly
"may result in lower acceptance rate".

Entirely excluded from quantization (`exclude_modules: ["mtp*"]`) — every
`mtp.*` tensor is BF16. That is why `--speculative-config moe_backend triton`
differs from the target's `marlin`: the draft cannot use the NVFP4 path at all
and falls through to the unquantized dispatcher.

Measured per-position acceptance from `run-0002`: 0.87 / 0.76 / 0.61.
...
**N=3 is optimal and beats N=2 by ~2%; N=4 loses. Do not spend a round on it.**

## What is actually NVFP4
...
Two-level scaling: per-block scale in FP8 E4M3 over groups of 16 along the
input dim, plus a per-tensor FP32 `weight_scale_2`. Checkpoint is 23.4 GB.

KV: the checkpoint declares FP8 and we pass `--kv-cache-dtype fp8`, so the flag
agrees with the checkpoint rather than fighting it. NVFP4 dequantizes to BF16
in-kernel — W4A16 is weight-only, so it buys bandwidth, not tensor-core
throughput.
...
|---|---|---|
| `--moe-backend marlin` | NVFP4 expert GEMMs only, all 40 layers. Alternatives offered: FLASHINFER_TRTLLM/CUTEDSL/CUTLASS, VLLM_CUTLASS, MARLIN, HUMMING, EMULATION. `triton`/`deepgemm` are **not** in the NVFP4 list | weight-only W4A16, good at low batch; CUTLASS/TRTLLM want higher arithmetic intensity and should win as batch rises |
| `--attention-backend flashinfer` | **10 of 40 layers.** The 30 linear-attn layers dispatch through `vllm::qwen_gdn_attention_core` and are untouched. Only FLASHINFER and TRITON_ATTN are offered | `decode_backend=xqa` is the low-batch fp8-KV decode kernel. Max blast radius on decode is a quarter of the model |
...
| `VLLM_MARLIN_USE_ATOMIC_ADD=1` | narrower than it looks: gates on `n < 2048 and k >= 2048`, so `gate_proj`/`up_proj` qualify and `down_proj`/`lm_head` do not | ~2 of 3 expert GEMMs; wins only at small batch |
...
## Roofline — how much headroom decode has
...
`run-0003` measured 71.07 tok/s with speculation off:

    71.07 x 2.548 GB = 181 GB/s = 66% of the 273 GB/s peak

**We are bandwidth-bound but not at the roofline.** A well-tuned memory-bound
kernel reaches 80-85%, so there is ~1.2-1.3x headroom in the non-spec path —
real, not transformative.
...
tokens. Cheapening the draft's vocab projection is the highest-leverage change
available and is architectural, not flag-exposed.

Second: the linear-attn projections are 1.016 GB — 40% of the forward — and
they are FP8, not NVFP4. At NVFP4 they would be ~0.5 GB. NVIDIA left the
single biggest weight block at 8 bits, and it is baked into the checkpoint.

## What changes from c1 to c10
...
throughput problem — where CUTLASS/TRTLLM should start beating Marlin and
`VLLM_MARLIN_USE_ATOMIC_ADD` should stop helping.
